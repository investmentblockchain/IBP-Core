    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // ibp: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid ICPro address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        // ICPro amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setAttribute(QNetworkRequest::User, BIP70_MESSAGE_PAYICPTREQUEST);
    netRequest.setUrl(url);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", BIP71_MIMETYPE_PAYICPTREQUEST);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    QNetworkRequest netRequest;
    netRequest.setAttribute(QNetworkRequest::User, BIP70_MESSAGE_PAYICPTACK);
    netRequest.setUrl(QString::fromStdString(details.payment_url()));
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, BIP71_MIMETYPE_PAYICPT);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", BIP71_MIMETYPE_PAYICPTACK);
...
void PaymentServer::netRequestFinished(QNetworkReply* reply)
{
    ...
    if (!verifySize(reply->size())) {
        Q_EMIT message(tr("Payment request rejected"),
            tr("Payment request %1 is too large (%2 bytes, allowed %3 bytes).")
                ...
                .arg(reply->size())
                .arg(BIP70_MAX_PAYICPTREQUEST_SIZE),
            CClientUIInterface::MSG_ERROR);
        return;
    }
    ...
    if (requestType == BIP70_MESSAGE_PAYICPTREQUEST)
    {
        PaymentRequestPlus request;
        SendCoinsRecipient recipient;
    ...
    else if (requestType == BIP70_MESSAGE_PAYICPTACK)
    {
...
{
    bool fVerified = (requestSize <= BIP70_MAX_PAYICPTREQUEST_SIZE);
    if (!fVerified) {
