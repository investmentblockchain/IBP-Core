...
*Setup instructions for a Gitian build of ICPro Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ICPro
...
Clone the git repositories for ICPro Core and Gitian.
...
in the ICPro Core repository under 'Fetch and build inputs' to install sources which require
...
To build ICPro Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ICPro Core repository.
