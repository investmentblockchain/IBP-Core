...
*Setup instructions for a Gitian build of IBP Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the IBP
...
- [Building IBP Core](#building-ibp-core)
...
Clone the git repositories for IBP Core and Gitian.
...
in the IBP Core repository under 'Fetch and build inputs' to install sources which require
...
To build IBP Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the IBP Core repository.
